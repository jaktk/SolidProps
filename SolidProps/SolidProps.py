...
def available_materials():
    """ return a sorted list of available material names """
    return sorted(MATERIAL_FILES.keys())
...
class SolidProps(object):
    ...
    def _set_df(self, material):
        if material.upper() not in MATERIAL_FILES:
            raise ValueError(
                f"Unknown material '{material}'. "
                f"Use available_materials() to list supported materials."
            )
        location = os.path.dirname(os.path.realpath(__file__))
        data = os.path.join(location, 'lib', MATERIAL_FILES[material.upper()])
    ...
    def get_cv(self, T):
        """
        return Cv in (J/(kg.K))
        for incompressible substances Cp = Cv
        """
